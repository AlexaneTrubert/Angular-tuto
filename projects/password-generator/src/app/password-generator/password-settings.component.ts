...
@Component({
  ...
  template: `
      ...
      <input [(ngModel)]="defaultSettings.length" (input)="onSettingsChange()" id="length" type="range" min="10"
             max="40" name="length">

      <label>
          <input [(ngModel)]="defaultSettings.uppercase" (change)="onSettingsChange()" role="switch" type="checkbox"
                 name="uppercase" id="uppercase">
          Contiendra des majuscules
      </label>

      <label>
          <input [(ngModel)]="defaultSettings.numbers" (change)="onSettingsChange()" role="switch" type="checkbox"
                 name="numbers" id="numbers">
          Contiendra des nombres
      </label>

      <label>
          <input [(ngModel)]="defaultSettings.symbols" (change)="onSettingsChange()" role="switch" type="checkbox"
